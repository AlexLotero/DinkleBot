...
class CarlaEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        if self.record_video:
            ...
            self.video_out = cv2.VideoWriter('../videos/carla_camera_feed.avi', fourcc, 20.0, (self.latest_image.shape[1], self.latest_image.shape[0]))
        else:
            self.video_out = None
...
def reset(self, **kwargs):
        ...
        # # Initialize Video Recording if Required
        # if self.record_video:
        ...
        # Attach or Reset Collision Sensor
        if self.collision_sensor is None or not self.collision_sensor.is_alive:
        ...
        # # Attach or Reset Camera
        # if self.camera is None or not self.camera.is_alive:
        ...
        # Check hero vehicle spawn
        hero_spawn_point = random.choice(self.world.get_map().get_spawn_points())
        self.vehicle = self.world.try_spawn_actor(self.hero_vehicle_bp, hero_spawn_point)
        if self.vehicle is None:
            raise RuntimeError("Failed to spawn hero vehicle.")
        else:
            print(f"Hero vehicle spawned at {hero_spawn_point.location}")

        # Check target vehicle spawn
        target_spawn_point = carla.Transform(
            carla.Location(
                x=hero_spawn_point.location.x + 10.0, 
                y=hero_spawn_point.location.y, 
        ...
        # Check camera spawn and attachment
        if self.camera is None or not self.camera.is_alive:
            self.camera = self.world.spawn_actor(self.camera_bp, self.camera_transform, attach_to=self.vehicle)
   ...
   def step(self, action):
    ...
    def compute_reward(self):
        ...
        # reward += max(0, 1.0 - speed_diff / target_speed)
        # reward = 0.1 * speed_reward + 1.0 * visual_maintained_reward - 1.0 * collision_penalty + - 0.5 * off_road_penalty + 1.0 * stability_reward
        reward = 1.0 * visual_maintained_reward - 1.0 * collision_penalty + - 0.5 * off_road_penalty + 1.0 * stability_reward
...
###############################################################################################
################### MK4 -- Code State at time of resumption ###################################
###############################################################################################
######## To Do ########
