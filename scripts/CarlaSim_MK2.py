...
class CarlaEnv(gym.Env):
    def __init__(self, carla_world, hero_vehicle_bp, target_vehicle_bp, vehicle_blueprints, collision_bp, camera_bp):
        super(CarlaEnv, self).__init__()
        self.world = carla_world
        self.hero_vehicle_bp = hero_vehicle_bp
        self.target_vehicle_bp = target_vehicle_bp
        self.vehicle_blueprints = vehicle_blueprints
        self.camera_bp = camera_bp
        self.collision_bp = collision_bp
        self.camera = None
        ...
        self.camera_transform = carla.Transform(carla.Location(x=1.5, z=2.4), carla.Rotation(pitch=-15))
    ...
    def reset(self, **kwargs):
        ...
        # Print reset start
        print("Starting episode reset")

        # Destroy Existing Actors for a Fresh Start
        ...
        # Reset Flags and Variables for New Episode
        ...
        self.vehicle = self.world.spawn_actor(self.hero_vehicle_bp, hero_spawn_point)
        if self.vehicle is None:
            raise RuntimeError("Failed to spawn hero vehicle.")
        print(f"Hero vehicle spawned at {hero_spawn_point.location}")

        # Set Up Target Vehicle in Front of Hero
        target_spawn_point = carla.Transform(
            carla.Location(
                x=hero_spawn_point.location.x + 10.0,
                y=hero_spawn_point.location.y,
                z=hero_spawn_point.location.z,
            ),
            hero_spawn_point.rotation,
        )
        self.target_vehicle = self.world.try_spawn_actor(self.target_vehicle_bp, target_spawn_point)
        if self.target_vehicle is None:
            print("Failed to spawn target vehicle.")
        else:
            print(f"Target vehicle spawned at {target_spawn_point.location}")

        # Spawn NPC Vehicles at Random Positions
        spawn_points = self.world.get_map().get_spawn_points()
        filtered_spawn_points = [sp for sp in spawn_points if sp.location != hero_spawn_point.location]
        for _ in range(50):
            spawn_point = random.choice(filtered_spawn_points)
            npc_bp = random.choice(self.vehicle_blueprints)
            self.world.try_spawn_actor(npc_bp, spawn_point)

        # Set Autopilot for NPC Vehicles
        self.vehicle.set_autopilot(False)  # Disable for hero vehicle
        for npc_vehicle in self.world.get_actors().filter('*vehicle*'):
            if npc_vehicle.id != self.vehicle.id:
                npc_vehicle.set_autopilot(True)

        # Attach Collision Sensor to Hero Vehicle
        ...
        if self.collision_sensor is None:
            ...
            print("Collision sensor attached to hero vehicle.")
        self.collision_sensor.listen(self.on_collision)

        # Attach Camera to Hero Vehicle
        self.camera = self.world.spawn_actor(self.camera_bp, self.camera_transform, attach_to=self.vehicle)
        if self.camera is None:
            raise RuntimeError("Failed to attach camera.")
        else:
            print("Camera successfully attached to hero vehicle.")
        self.camera.listen(self.process_camera_image)
    ...
    def step(self, action):
        ...
        # Optional truncation flag (not used here)
        truncated = False
        info = {}

        # Debugging step information
        print(f"Step: {self.current_step}, Reward: {reward}, Done: {done}")
    ...
    def compute_reward(self):
        ...
        if self.is_on_road():
            reward += 1.0  # Reward for staying on the road
        else:
            reward -= 2.0  # Penalty for going off-road
        ...
        # Reward for maintaining visual on target vehicle
        visual_maintained_reward = self.compute_visual_maintained()
        reward += 1.0 * visual_maintained_reward

        # Reward for tracking stability
        stability_reward = self.compute_tracking_stability()
        reward += 1.0 * stability_reward

        # Debugging reward computation
        print(f"Computed Reward: {reward}, Visual Maintained: {visual_maintained_reward}, Stability: {stability_reward}")
...
carla_env = CarlaEnv(world, hero_vehicle_bp, target_vehicle_bp, filtered_blueprints, collision_bp, camera_bp)
...
print("Testing first reset")
obs = env.reset()

# print("First reset successful")
# print("Testing second reset")
# obs = env.reset()
...
callback = TrainingMonitorCallback(n_steps=2048, total_timesteps=100000, env=carla_env, verbose=1)
